...
class SpriteGenerator {
    ...
    static generateHumanoidSprite(type, frame, facing = 'south', animType = 'idle') {
        ...
        // Fallout 2 palette: very muted, earth tones, dark outlines
        const palettes = {
        ...
        const outline = (arr) => rgb(arr, -40);

        // ---- ANIMATION OFFSETS ----
        // Walk cycle: 4 frames, legs stride forward/back, arms swing opposite
        // Frame 0: neutral, 1: left forward, 2: neutral, 3: right forward
        const walkCycle = [
            { leftLegX: 0, leftLegY: 0, rightLegX: 0, rightLegY: 0, leftArmY: 0, rightArmY: 0 },
            { leftLegX: -2, leftLegY: -1, rightLegX: 2, rightLegY: 1, leftArmY: 2, rightArmY: -2 },
            { leftLegX: 0, leftLegY: 0, rightLegX: 0, rightLegY: 0, leftArmY: 0, rightArmY: 0 },
            { leftLegX: 2, leftLegY: 1, rightLegX: -2, rightLegY: -1, leftArmY: -2, rightArmY: 2 },
        ];
        // Attack cycle: arm raised and swinging
        const attackCycle = [
            { rightArmAngle: 0, rightArmExt: 0, leftArmY: 0 },
            { rightArmAngle: -0.8, rightArmExt: -6, leftArmY: 1 },
            { rightArmAngle: 0.5, rightArmExt: 4, leftArmY: -1 },
            { rightArmAngle: 0, rightArmExt: 0, leftArmY: 0 },
        ];

        const wk = walkCycle[frame % 4];
        const atk = attackCycle[frame % 4];
        const isWalking = animType === 'walk';
        const isAttacking = animType === 'attack';

        // ---- LEGS (with walk animation) ----
        const legW = 4 * sc, legH = 10 * sc;
        const legY = baseY - legH;
        const lLegOffX = isWalking ? wk.leftLegX * sc : 0;
        const lLegOffY = isWalking ? wk.leftLegY * sc : 0;
        const rLegOffX = isWalking ? wk.rightLegX * sc : 0;
        const rLegOffY = isWalking ? wk.rightLegY * sc : 0;

        // Left leg
        ctx.fillStyle = rgb(p.pants);
        ctx.fillRect(cx - 5 * sc + lLegOffX, legY + lLegOffY, legW, legH - lLegOffY);
        // Left leg outline
        ctx.strokeStyle = outline(p.pants);
        ctx.lineWidth = 0.8;
        ctx.strokeRect(cx - 5 * sc + lLegOffX, legY + lLegOffY, legW, legH - lLegOffY);
        // Right leg
        ctx.fillStyle = rgb(p.pants, -5);
        ctx.fillRect(cx + 1 * sc + rLegOffX, legY + rLegOffY, legW, legH - rLegOffY);
        ctx.strokeStyle = outline(p.pants);
        ctx.strokeRect(cx + 1 * sc + rLegOffX, legY + rLegOffY, legW, legH - rLegOffY);
        // Knee highlights
        ctx.fillStyle = rgb(p.pants, 10);
        ctx.fillRect(cx - 5 * sc + lLegOffX, legY + legH * 0.35 + lLegOffY, legW, 2);
        ctx.fillRect(cx + 1 * sc + rLegOffX, legY + legH * 0.35 + rLegOffY, legW, 2);

        // ---- BOOTS ----
        const bootH = 5 * sc;
        // Left boot
        ctx.fillStyle = rgb(p.boots);
        ctx.fillRect(cx - 5.5 * sc + lLegOffX, baseY - bootH + lLegOffY, legW + 2 * sc, bootH);
        ctx.strokeStyle = outline(p.boots);
        ctx.lineWidth = 0.8;
        ctx.strokeRect(cx - 5.5 * sc + lLegOffX, baseY - bootH + lLegOffY, legW + 2 * sc, bootH);
        // Right boot
        ctx.fillStyle = rgb(p.boots);
        ctx.fillRect(cx + 0.5 * sc + rLegOffX, baseY - bootH + rLegOffY, legW + 2 * sc, bootH);
        ctx.strokeStyle = outline(p.boots);
        ctx.strokeRect(cx + 0.5 * sc + rLegOffX, baseY - bootH + rLegOffY, legW + 2 * sc, bootH);
        // Boot soles (dark line)
        ctx.fillStyle = '#080808';
        ctx.fillRect(cx - 5.5 * sc + lLegOffX, baseY - 1.5 + lLegOffY, legW + 2 * sc, 1.5);
        ctx.fillRect(cx + 0.5 * sc + rLegOffX, baseY - 1.5 + rLegOffY, legW + 2 * sc, 1.5);

        // ---- TORSO ----
        const bodyW = 14 * sc, bodyH = 16 * sc;
        const torsoY = legY - bodyH + 4;
        ctx.fillStyle = rgb(p.shirt);
        ctx.fillRect(cx - bodyW / 2, torsoY, bodyW, bodyH);
        // Torso shading
        ctx.fillStyle = rgba(p.shirt, 0.18, 18);
        ctx.fillRect(cx - bodyW / 2, torsoY, bodyW * 0.35, bodyH);
        ctx.fillStyle = rgba(p.shirt, 0.14, -15);
        ctx.fillRect(cx + bodyW * 0.15, torsoY, bodyW * 0.35, bodyH);
        // Torso outline
        ctx.strokeStyle = outline(p.shirt);
        ctx.lineWidth = 0.8;
        ctx.strokeRect(cx - bodyW / 2, torsoY, bodyW, bodyH);
        // Center seam
        ...
        ctx.fillRect(cx - bodyW / 2, torsoY + bodyH - 3, bodyW, 4);
        ctx.fillStyle = rgb(p.accent);
        ctx.fillRect(cx - 2, torsoY + bodyH - 3, 4, 4);
        if (type === 'player' || type === 'guard' || type === 'raider') {
            ctx.fillStyle = rgb(p.belt, -10);
            ctx.fillRect(cx - bodyW / 2 + 1, torsoY + bodyH - 2, 3, 3.5);
            ctx.fillRect(cx + bodyW / 2 - 4, torsoY + bodyH - 2, 3, 3.5);
        }

        // ---- LEFT ARM (with swing animation) ----
        const armW = 4 * sc, armH = 13 * sc;
        const armY = torsoY + 3;
        const lArmOff = isWalking ? wk.leftArmY * sc : (isAttacking ? atk.leftArmY * sc : 0);

        ctx.save();
        if (isWalking || isAttacking) {
            // Slight rotation for arm swing
            const armAngle = isWalking ? (wk.leftArmY * 0.04) : 0;
            ctx.translate(cx - bodyW / 2, armY + 2);
            ctx.rotate(armAngle);
            ctx.translate(-(cx - bodyW / 2), -(armY + 2));
        }
        ctx.fillStyle = rgb(p.shirt);
        ctx.fillRect(cx - bodyW / 2 - armW, armY + lArmOff, armW, armH);
        ctx.strokeStyle = outline(p.shirt);
        ctx.lineWidth = 0.6;
        ctx.strokeRect(cx - bodyW / 2 - armW, armY + lArmOff, armW, armH);
        // Rolled sleeve
        ctx.fillStyle = rgb(p.shirt, 12);
        ctx.fillRect(cx - bodyW / 2 - armW, armY + lArmOff, armW, 3);
        // Left hand
        const handH = 3 * sc;
        ctx.fillStyle = rgb(p.skin);
        ctx.fillRect(cx - bodyW / 2 - armW, armY + armH + lArmOff, armW, handH);
        ctx.strokeStyle = outline(p.skin);
        ctx.lineWidth = 0.5;
        ctx.strokeRect(cx - bodyW / 2 - armW, armY + armH + lArmOff, armW, handH);
        ctx.restore();

        // ---- RIGHT ARM (attack swing or walk swing) ----
        const rArmOff = isWalking ? wk.rightArmY * sc : 0;

        ctx.save();
        if (isAttacking) {
            // Attack: arm rotates for swing
            const pivotX = cx + bodyW / 2;
            const pivotY = armY + 2;
            ctx.translate(pivotX, pivotY);
            ctx.rotate(atk.rightArmAngle);
            ctx.translate(-pivotX, -pivotY);
        } else if (isWalking) {
            const armAngle = wk.rightArmY * 0.04;
            ctx.translate(cx + bodyW / 2, armY + 2);
            ctx.rotate(armAngle);
            ctx.translate(-(cx + bodyW / 2), -(armY + 2));
        }
        ctx.fillStyle = rgb(p.shirt, -3);
        ctx.fillRect(cx + bodyW / 2, armY + rArmOff + (isAttacking ? atk.rightArmExt : 0), armW, armH);
        ctx.strokeStyle = outline(p.shirt);
        ctx.lineWidth = 0.6;
        ctx.strokeRect(cx + bodyW / 2, armY + rArmOff + (isAttacking ? atk.rightArmExt : 0), armW, armH);
        // Rolled sleeve
        ctx.fillStyle = rgb(p.shirt, 12);
        ctx.fillRect(cx + bodyW / 2, armY + rArmOff + (isAttacking ? atk.rightArmExt : 0), armW, 3);
        // Right hand
        ctx.fillStyle = rgb(p.skin);
        const rHandY = armY + armH + rArmOff + (isAttacking ? atk.rightArmExt : 0);
        ctx.fillRect(cx + bodyW / 2, rHandY, armW, handH);
        ctx.strokeStyle = outline(p.skin);
        ctx.lineWidth = 0.5;
        ctx.strokeRect(cx + bodyW / 2, rHandY, armW, handH);

        // ---- WEAPON (moves with right arm) ----
        if (type === 'player' || type === 'raider' || type === 'guard') {
            const wx = cx + bodyW / 2 + armW / 2;
            const wy = rHandY + handH;
        ...
        if (type === 'elder') {
            ...
            ctx.lineTo(sx, torsoY - 18);
            ctx.stroke();
            ctx.fillStyle = '#503870';
            ctx.fillRect(sx - 2, torsoY - 18, 4, 5);
            ctx.fillStyle = '#7755AA';
            ctx.beginPath();
            ctx.moveTo(sx, torsoY - 22);
            ctx.lineTo(sx + 2.5, torsoY - 18);
            ctx.lineTo(sx - 2.5, torsoY - 18);
            ctx.closePath();
            ctx.fill();
        }
        ...
        if (type === 'merchant') {
            ...
            ctx.beginPath();
            ctx.ellipse(px, armY + armH * 0.4, 5.5, 4, 0, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(px + 3, armY + armH * 0.2);
            ctx.lineTo(cx - bodyW / 2, torsoY + 4);
            ctx.stroke();
            ctx.fillStyle = '#555';
            ctx.fillRect(px - 1, armY + armH * 0.2 - 3, 1.5, 3);
        }

        // ---- NECK ----
        ctx.fillStyle = rgb(p.skin);
        ctx.fillRect(cx - 2 * sc, torsoY - 2, 4 * sc, 4);
        ctx.fillStyle = rgba(p.skin, 0.2, -20);
        ctx.fillRect(cx - 2 * sc, torsoY, 4 * sc, 2);

        // ---- HEAD ----
        const headR = 6 * sc;
        const headY = torsoY - 2 - headR * 1.2;

        // Head shape with dark outline (Fallout 2 style)
        ctx.fillStyle = rgb(p.skin);
        ctx.beginPath();
        ...
        if (facing === 'north') {
            ...
            // Eyes (slightly larger, more defined - Fallout 2 style)
            ctx.fillStyle = '#CCC';
            ctx.beginPath();
            ctx.ellipse(cx - 2.5 * sc, headY - 0.5, 1.8, 1.2, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(cx + 2.5 * sc, headY - 0.5, 1.8, 1.2, 0, 0, Math.PI * 2);
            ctx.fill();
            // Eye outlines (Fallout 2 has dark outlines around everything)
            ...
            ctx.beginPath();
            ctx.arc(cx - 2.2 * sc, headY - 0.3, 0.9, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(cx + 2.8 * sc, headY - 0.3, 0.9, 0, Math.PI * 2);
            ctx.fill();
            // Pupils
            ctx.fillStyle = '#0A0A0A';
            ctx.beginPath();
            ctx.arc(cx - 2.2 * sc, headY - 0.3, 0.45, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(cx + 2.8 * sc, headY - 0.3, 0.45, 0, Math.PI * 2);
            ctx.fill();
            // Eye highlights
            ctx.fillStyle = 'rgba(255,255,255,0.35)';
            ctx.beginPath();
            ctx.arc(cx - 2.5 * sc, headY - 0.7, 0.35, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(cx + 2.5 * sc, headY - 0.7, 0.35, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.lineWidth = 0.7;
            ctx.beginPath();
            ctx.moveTo(cx - 1.8 * sc, headY + 3.2);
            ctx.lineTo(cx + 1.8 * sc, headY + 3.2);
            ctx.stroke();
            // Scars
            if (type === 'raider') {
        ...
        // Head outline (darker for Fallout 2 feel)
        ctx.strokeStyle = rgba(p.skin, 0.5, -35);
        ctx.lineWidth = 0.7;
        ctx.beginPath();
        ctx.ellipse(cx, headY, headR, headR * 1.1, 0, 0, Math.PI * 2);
        ctx.stroke();
        ...
        // Full body outline pass (subtle dark border for pixel art look)
        // This gives the Fallout 2 pre-rendered 3D model feel
...
class IsometricRenderer {
    ...
    drawEntity(gx, gy, entityType, facing = 'south', isSelected = false, hp = null, maxHp = null, animState = 'idle') {
        const screen = this.worldToScreen(gx, gy);
        const ctx = this.ctx;
        const z = this.camera.zoom;
        ...
        for (const item of renderList) {
            if (item.type === 'tile') {
                ...
                this.drawEntity(ent.x, ent.y, ent.spriteType || ent.type, ent.facing || 'south',
                    ent === gameState.selectedEntity, ent.stats ? ent.stats.hp : null, ent.stats ? ent.stats.maxHp : null, ent.animState || 'idle');
            }
        }
