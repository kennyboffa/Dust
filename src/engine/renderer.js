...
// Dark, gritty pre-rendered aesthetic with muted earth tones,
// heavy shadows, environmental decay, and retro-futuristic detail
// ============================================
...
class IsometricRenderer {
    ...
    // ---- TILE RENDERING (Fallout 2 style) ----
    ...
    drawTile(gx, gy, tileType, height = 0) {
        ...
        // Per-tile color variation (darker, grittier)
        const variation = (n - 0.5) * 8;

        // Directional shadow: tiles cast shadow to bottom-right
        const inShadow = height === 0 && n3 > 0.7;
        const shadowDarken = inShadow ? -8 : 0;
        ...
        ctx.lineTo(screen.x - tw, screen.y - hOffset);
        ctx.closePath();
        ctx.fillStyle = this.adjustColor(colors.top, variation + shadowDarken);
        ctx.fill();

        // Dithering noise overlay (pre-rendered sprite look)
        ctx.save();
        ctx.clip();
        // Speckle dithering
        for (let i = 0; i < 6; i++) {
            const dx = this.noise(gx * 13 + i, gy * 17) - 0.5;
            const dy = this.noise(gx * 17 + i, gy * 13) - 0.5;
            const dark = this.noise(gx + i * 7, gy + i * 3) > 0.5;
            ctx.fillStyle = dark ? `rgba(0,0,0,${0.04 + n * 0.04})` : `rgba(255,240,200,${0.02 + n2 * 0.02})`;
            ctx.fillRect(screen.x + dx * tw * 1.4, screen.y - hOffset + dy * th * 1.4, z * 1.5, z);
        }

        this.drawTileTexture(ctx, screen, tw, th, hOffset, z, gx, gy, tileType, n, n2);

        // Ground shadow from nearby walls (ambient occlusion fake)
        const aoStrength = 0.06 + n * 0.04;
        ctx.fillStyle = `rgba(0,0,0,${aoStrength})`;
        ctx.fillRect(screen.x - tw, screen.y - th - hOffset, tw * 2, th * 2);

        ctx.restore();

        // Edge lines (darker, more defined for pre-rendered look)
        ctx.beginPath();
        ctx.moveTo(screen.x, screen.y - th - hOffset);
        ctx.lineTo(screen.x + tw, screen.y - hOffset);
        ctx.lineTo(screen.x, screen.y + th - hOffset);
        ctx.lineTo(screen.x - tw, screen.y - hOffset);
        ctx.closePath();
        ctx.strokeStyle = this.adjustColor(colors.stroke, variation - 5);
        ctx.lineWidth = 0.7;
        ctx.stroke();
