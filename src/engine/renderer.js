...
class SpriteGenerator {
    ...
    static generateHumanoidSprite(type, frame, facing = 'south', animType = 'idle') {
        const w = 48, h = 64;
        const c = this.createCanvas(w, h);
        const ctx = c.getContext('2d');
        ...
        // Fallout 2 palette: warm skin tones, muted clothing, tribal/wasteland feel
        const palettes = {
            'player':  { skin:[165,128,90], shirt:[100,78,42], pants:[55,42,25], boots:[30,20,10], hair:[38,26,16], belt:[42,32,14], accent:[120,100,48], armor: true, bare: false },
            'villager':{ skin:[155,120,85], shirt:[140,110,75], pants:[85,70,50], boots:[35,28,18], hair:[60,45,30], belt:[50,40,28], accent:[90,75,55], armor: false, bare: true },
            'merchant':{ skin:[150,118,82], shirt:[95,72,38], pants:[48,38,25], boots:[32,22,12], hair:[55,42,28], belt:[50,38,25], accent:[120,98,58], armor: false, bare: false },
            'elder':   { skin:[145,115,82], shirt:[68,48,85], pants:[32,26,45], boots:[22,15,25], hair:[150,148,145], belt:[40,32,55], accent:[85,62,118], armor: false, bare: false },
            'guard':   { skin:[155,120,85], shirt:[55,72,35], pants:[35,45,22], boots:[22,22,10], hair:[38,28,16], belt:[45,45,25], accent:[65,78,42], armor: true, bare: false },
            'raider':  { skin:[150,115,80], shirt:[72,22,18], pants:[32,15,15], boots:[22,10,10], hair:[25,25,25], belt:[55,28,25], accent:[85,28,25], armor: true, bare: true },
        ...
        const ol = (arr) => rgb(arr, -50); // strong dark outlines like FO2
        ...
        // Measurements - stockier, wider shoulders like FO2
        const bodyW = 16 * sc, bodyH = 14 * sc;
        const legW = 5 * sc, legH = 9 * sc;
        const armW = 4.5 * sc, armH = 11 * sc;
        const headR = 5.5 * sc;
        const legY = baseY - legH;
        const torsoY = legY - bodyH + 3;
        const armY = torsoY + 2;

        // ---- LEGS (rounded, thick - FO2 style) ----
        ...
        ctx.beginPath();
        ctx.ellipse(cx - 3.5 * sc + llx, legY + legH / 2 + lly, legW / 2, legH / 2 - lly / 2, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.pants);
        ctx.lineWidth = 1;
        ctx.stroke();
        // Right leg
        ctx.fillStyle = rgb(p.pants, -8);
        ctx.beginPath();
        ctx.ellipse(cx + 3.5 * sc + rlx, legY + legH / 2 + rly, legW / 2, legH / 2 - rly / 2, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.pants);
        ctx.stroke();

        // ---- BOOTS (chunky, rounded) ----
        ctx.fillStyle = rgb(p.boots);
        ctx.beginPath();
        ctx.ellipse(cx - 3.5 * sc + llx, baseY - 2 + lly, legW / 2 + 1, 3 * sc, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.boots);
        ctx.lineWidth = 0.8;
        ctx.stroke();
        ctx.fillStyle = rgb(p.boots);
        ctx.beginPath();
        ctx.ellipse(cx + 3.5 * sc + rlx, baseY - 2 + rly, legW / 2 + 1, 3 * sc, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.boots);
        ctx.stroke();

        // ---- TORSO (wide, rounded - FO2 stocky build) ----
        const torsoColor = p.bare ? p.skin : p.shirt;
        ctx.fillStyle = rgb(torsoColor);
        ctx.beginPath();
        ctx.ellipse(cx, torsoY + bodyH / 2, bodyW / 2, bodyH / 2, 0, 0, Math.PI * 2);
        ctx.fill();
        // Light side shading
        ctx.fillStyle = rgba(torsoColor, 0.2, 20);
        ctx.beginPath();
        ctx.ellipse(cx - 2, torsoY + bodyH / 2 - 1, bodyW / 4, bodyH / 3, -0.2, 0, Math.PI * 2);
        ctx.fill();
        // Dark side
        ctx.fillStyle = rgba(torsoColor, 0.15, -20);
        ctx.beginPath();
        ctx.ellipse(cx + 3, torsoY + bodyH / 2 + 1, bodyW / 4, bodyH / 3, 0.2, 0, Math.PI * 2);
        ctx.fill();
        // Torso outline
        ctx.strokeStyle = ol(torsoColor);
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ctx.ellipse(cx, torsoY + bodyH / 2, bodyW / 2, bodyH / 2, 0, 0, Math.PI * 2);
        ctx.stroke();

        // Muscle definition for bare-chested types
        if (p.bare && facing !== 'north') {
            ctx.strokeStyle = rgba(p.skin, 0.15, -25);
            ctx.lineWidth = 0.6;
            // Pec line
            ctx.beginPath();
            ctx.moveTo(cx - 4 * sc, torsoY + bodyH * 0.3);
            ctx.quadraticCurveTo(cx, torsoY + bodyH * 0.45, cx + 4 * sc, torsoY + bodyH * 0.3);
            ctx.stroke();
            // Abs hint
            ctx.beginPath();
            ctx.moveTo(cx, torsoY + bodyH * 0.4);
            ctx.lineTo(cx, torsoY + bodyH * 0.75);
            ctx.stroke();
        }

        // Armor/vest overlay
        if (p.armor && !p.bare) {
            const vc = type === 'raider' ? [45,25,20] : type === 'guard' ? [58,55,38] : [70,60,35];
            ctx.fillStyle = rgba(vc, 0.55);
            ctx.beginPath();
            ctx.ellipse(cx, torsoY + bodyH / 2, bodyW / 2 - 1, bodyH / 2 - 1, 0, 0, Math.PI * 2);
            ctx.fill();
        }

        // Raider leather straps across bare chest
        if (type === 'raider') {
            ctx.strokeStyle = '#3A1A12';
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(cx - bodyW / 2 + 2, torsoY + 3);
            ctx.lineTo(cx + bodyW / 2 - 2, torsoY + bodyH - 3);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(cx + bodyW / 2 - 2, torsoY + 3);
            ctx.lineTo(cx - bodyW / 2 + 2, torsoY + bodyH - 3);
        ...
        ctx.fillRect(cx - bodyW / 2 + 1, torsoY + bodyH - 3, bodyW - 2, 3);
        ctx.fillStyle = rgb(p.accent);
        ctx.fillRect(cx - 1.5, torsoY + bodyH - 3, 3, 3);

        // ---- LEFT ARM (skin visible - FO2 style) ----
        const lArmOff = walking ? wk.laOff * sc : (attacking ? (at.ext < 0 ? 1 : 0) : 0);
        ctx.save();
        if (walking) {
            ctx.translate(cx - bodyW / 2, armY + 2);
            ctx.rotate(wk.laOff * 0.04);
            ctx.translate(-(cx - bodyW / 2), -(armY + 2));
        }
        // Upper arm (skin)
        ctx.fillStyle = rgb(p.skin);
        ctx.beginPath();
        ctx.ellipse(cx - bodyW / 2 - armW / 2 + 1, armY + armH * 0.35 + lArmOff, armW / 2, armH * 0.4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.9;
        ctx.stroke();
        // Forearm (slightly darker)
        ctx.fillStyle = rgb(p.skin, -8);
        ctx.beginPath();
        ctx.ellipse(cx - bodyW / 2 - armW / 2 + 1, armY + armH * 0.75 + lArmOff, armW / 2 - 0.3, armH * 0.3, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.7;
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.arc(cx - bodyW / 2 - armW / 2 + 1, armY + armH + lArmOff, 2 * sc, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.6;
        ctx.stroke();
        ...
        // ---- RIGHT ARM (weapon arm - with attack animation) ----
        ...
        ctx.fillStyle = rgb(p.skin);
        ctx.beginPath();
        ctx.ellipse(cx + bodyW / 2 + armW / 2 - 1, armY + armH * 0.35 + rArmOff + rArmExt, armW / 2, armH * 0.4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.9;
        ctx.stroke();
        ...
        ctx.fillStyle = rgb(p.skin, -8);
        ctx.beginPath();
        ctx.ellipse(cx + bodyW / 2 + armW / 2 - 1, armY + armH * 0.75 + rArmOff + rArmExt, armW / 2 - 0.3, armH * 0.3, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.7;
        ctx.stroke();
        ...
        ctx.fillStyle = rgb(p.skin, -5);
        ctx.beginPath();
        ctx.arc(cx + bodyW / 2 + armW / 2 - 1, rHandY, 2 * sc, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.6;
        ctx.stroke();
        ...
        if (type === 'player' || type === 'raider' || type === 'guard') {
            const wx = cx + bodyW / 2 + armW / 2 - 1;
            const wy = rHandY + 2 * sc;
            if (type === 'player' || type === 'guard') {
                ctx.fillStyle = '#6A6A6A';
                ctx.beginPath();
                ctx.moveTo(wx - 0.5, wy);
                ctx.lineTo(wx + 0.3, wy - 8);
                ctx.lineTo(wx + 1, wy);
                ctx.closePath();
                ctx.fill();
                ctx.fillStyle = '#2A1C0C';
                ctx.fillRect(wx - 0.7, wy, 1.4, 3);
            } else {
                ctx.fillStyle = '#444';
                ctx.fillRect(wx - 0.7, wy - 10, 1.4, 10);
                ctx.fillStyle = '#555';
                ctx.fillRect(wx - 1, wy, 2, 3);
            }
        }
        ctx.restore();

        // Elder staff (drawn outside arm transform)
        if (type === 'elder') {
            ...
            ctx.beginPath();
            ctx.arc(sx, torsoY - 16, 2.5, 0, Math.PI * 2);
            ctx.fill();
        }
        ...
        if (type === 'merchant') {
            ...
            ctx.beginPath();
            ctx.ellipse(cx - bodyW / 2 - 3, armY + armH * 0.4, 5, 3.5, 0, 0, Math.PI * 2);
            ctx.fill();
        ...
        // ---- NECK (thick, visible) ----
        ctx.fillStyle = rgb(p.skin);
        ctx.fillRect(cx - 2.5 * sc, torsoY - 2, 5 * sc, 4);
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 0.5;
        ctx.beginPath();
        ctx.moveTo(cx - 2.5 * sc, torsoY - 2);
        ctx.lineTo(cx - 2.5 * sc, torsoY + 2);
        ctx.moveTo(cx + 2.5 * sc, torsoY - 2);
        ctx.lineTo(cx + 2.5 * sc, torsoY + 2);
        ctx.stroke();

        // ---- HEAD (larger, rounder - FO2 proportions) ----
        ...
        ctx.fill();
        // Light shading on head
        ctx.fillStyle = rgba(p.skin, 0.15, 18);
        ctx.beginPath();
        ctx.ellipse(cx - 1.5, headY - 1, headR * 0.5, headR * 0.6, -0.2, 0, Math.PI * 2);
        ctx.fill();
        // Strong head outline
        ctx.strokeStyle = ol(p.skin);
        ctx.lineWidth = 1.2;
        ...
        if (facing === 'north') {
            ...
            if (!isMutant) {
                ...
                ctx.beginPath();
                ctx.ellipse(cx, headY, headR + 0.5, headR * 1.0, 0, 0, Math.PI * 2);
                ctx.fill();
                ctx.strokeStyle = ol(p.hair);
                ctx.lineWidth = 0.8;
                ctx.stroke();
            }
            // Ears
            ctx.fillStyle = rgb(p.skin, -5);
            ctx.beginPath();
            ctx.ellipse(cx - headR, headY, 1.5, 2.5, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(cx + headR, headY, 1.5, 2.5, 0, 0, Math.PI * 2);
            ctx.fill();
            ...
            if (!isMutant) {
                ctx.fillStyle = rgb(p.hair);
                ctx.beginPath();
                ctx.ellipse(cx, headY - 2, headR + 0.5, headR * 0.6, 0, Math.PI, Math.PI * 2);
                ctx.fill();
                ctx.fillRect(cx - headR - 0.5, headY - 2, 2, 5);
                ctx.fillRect(cx + headR - 1.5, headY - 2, 2, 5);
            }
            // Eyes (small, dark - FO2 style at this scale)
            ctx.fillStyle = '#DDD';
            ctx.beginPath();
            ctx.ellipse(cx - 2 * sc, headY - 0.3, 1.4, 0.9, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(cx + 2 * sc, headY - 0.3, 1.4, 0.9, 0, 0, Math.PI * 2);
            ctx.fill();
            // Irises
            ctx.fillStyle = isMutant ? '#8AA800' : '#221808';
            ctx.beginPath();
            ctx.arc(cx - 1.8 * sc, headY - 0.2, 0.7, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(cx + 2.2 * sc, headY - 0.2, 0.7, 0, Math.PI * 2);
            ctx.fill();
            // Nose shadow
            ctx.fillStyle = rgba(p.skin, 0.4, -18);
            ctx.beginPath();
            ctx.moveTo(cx, headY);
            ...
            ctx.fill();
            // Mouth
            ctx.strokeStyle = rgba(p.skin, 0.6, -30);
            ctx.lineWidth = 0.6;
            ctx.beginPath();
            ctx.moveTo(cx - 1.5 * sc, headY + 3);
            ctx.lineTo(cx + 1.5 * sc, headY + 3);
            ctx.stroke();
            // Scars for raider
            if (type === 'raider') {
        ...
        if (type === 'raider') {
            ...
            for (let i = 0; i < 4; i++) {
                ctx.fillRect(cx - 0.5 + (rng() - 0.5) * 2, headY - headR - 1 - i * 1.5, 1, 2);
            }
        }
        ...
        if (type === 'guard') {
            ...
            ctx.beginPath();
            ctx.ellipse(cx, headY - 1, headR + 1.5, headR * 0.65, 0, Math.PI, Math.PI * 2);
            ctx.fill();
...
class IsometricRenderer {
    ...
    _generateAllSprites() {
        ...
        // Humanoid sprites: idle, walk, attack frames x 4 directions
        ...
        for (const type of humanoidTypes) {
            ...
            for (let f = 0; f < 4; f++) {
                // Idle frames
                const south = SpriteGenerator.generateHumanoidSprite(type, f, 'south', 'idle');
                const north = SpriteGenerator.generateHumanoidSprite(type, f, 'north', 'idle');
                ...
                this._humanoidSprites[type].west.push(south);

                // Walk frames
                const walkSouth = SpriteGenerator.generateHumanoidSprite(type, f, 'south', 'walk');
                const walkNorth = SpriteGenerator.generateHumanoidSprite(type, f, 'north', 'walk');
                const walkEast = SpriteGenerator.mirrorSprite(walkSouth);
                this._humanoidSprites[type].walk_south.push(walkSouth);
                this._humanoidSprites[type].walk_north.push(walkNorth);
                this._humanoidSprites[type].walk_east.push(walkEast);
                this._humanoidSprites[type].walk_west.push(walkSouth);

                // Attack frames
                const atkSouth = SpriteGenerator.generateHumanoidSprite(type, f, 'south', 'attack');
                const atkNorth = SpriteGenerator.generateHumanoidSprite(type, f, 'north', 'attack');
                const atkEast = SpriteGenerator.mirrorSprite(atkSouth);
                this._humanoidSprites[type].attack_south.push(atkSouth);
                this._humanoidSprites[type].attack_north.push(atkNorth);
                this._humanoidSprites[type].attack_east.push(atkEast);
                this._humanoidSprites[type].attack_west.push(atkSouth);
            }
        }
    ...
    drawEntity(gx, gy, entityType, facing = 'south', isSelected = false, hp = null, maxHp = null, animState = 'idle', inCombat = false) {
        const screen = this.worldToScreen(gx, gy);
        const ctx = this.ctx;
        const z = this.camera.zoom;
        ...
        if (['rat', 'scorpion', 'cave_spider'].includes(entityType)) {
            ...
            // Use pre-rendered humanoid sprite (directional with animation state)
        ...
        for (const item of renderList) {
            if (item.type === 'tile') {
                ...
                this.drawEntity(ent.x, ent.y, ent.spriteType || ent.type, ent.facing || 'south',
                    ent === gameState.selectedEntity, ent.stats ? ent.stats.hp : null, ent.stats ? ent.stats.maxHp : null, ent.animState || 'idle', gameState.inCombat);
            }
        }
